...
(0, _lodash.assign)(QueryCompiler_MSSQL.prototype, {
  ...
  _returning: function _returning(method, value) {
    var buildInto = function buildInto() {
    ...
    switch (method) {
      case 'update':
      case 'insert':
        return value ? 'output ' + this.formatter.columnizeWithPrefix('inserted.', value) + ' ' + buildInto() : '';
      case 'del':
        var tmpTable = '#ident_insert_' + Date.now();
        return value ? 'output ' + this.formatter.columnizeWithPrefix('deleted.', value) + ' ' + buildInto() : '';
      case 'rowcount':
