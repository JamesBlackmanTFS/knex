...
(0, _lodash.assign)(QueryCompiler_MSSQL.prototype, {
  ...
  insert: function insert() {
    ...
    var postReturningSql = returning ? this._postReturning('insert', returning) + ' ' : '';
  ...
  update: function update() {
    ...
    return {
      sql: this.with() + ('update ' + (top ? top + ' ' : '') + this.tableName) + ' set ' + updates.join(', ') + (returning ? ' ' + this._returning('update', returning) : '') + (join ? ' from ' + this.tableName + ' ' + join : '') + (where ? ' ' + where : '') + (order ? ' ' + order : '') + (returning ? ' ' + this._postReturning('update', returning) : '') + (!returning ? this._returning('rowcount', '@@rowcount') : ''),
  ...
  del: function del() {
    ...
    return {
      sql: this.with() + ('delete from ' + tableName) + (returning ? ' ' + this._returning('del', returning) : '') + (wheres ? ' ' + wheres : '') + (returning ? ' ' + this._postReturning('del', returning) : '') + (!returning ? this._returning('rowcount', '@@rowcount') : ''),
      returning: returning || '@@rowcount'
    };
  },
  ...
  _postReturning: function _postReturning(method, value) {
